...
/** Extract component for the mrbextractor module. */
class Mrbextractor_ExtractComponent extends AppComponent
{
    /**
     * Extract mrb information.
     *
     * @param array|ItemRevisionModel $revision
     */
    public function extractMRB($revision)
    {
        ...
        // Extract information.
        exec(
            "cd $bitstreamsFolder && ".$pythonExecutable.' '.dirname(
                __FILE__
            ).'/scripts/mrb-extract-views.py  "'.$bitstreamsFolder.'/'.$mrbFound.'"  "'.$bitstreamsFolder.'" ',
            $output
        );
        ...
        if (file_exists($bitstreamsFolder.'/index.json')) {
            ...
            $lastRevision = MidasLoader::loadModel('Item')->getLastRevision($item);
            $metadataDao = MidasLoader::loadModel('Metadata')->getMetadata(
                MIDAS_METADATA_TEXT,
                'mrbextrator',
                'slicerdatastore'
            );
            if (!$metadataDao) {
                MidasLoader::loadModel('Metadata')->addMetadata(
                    MIDAS_METADATA_TEXT,
                    'mrbextrator',
                    'slicerdatastore',
                    ''
            ...
            MidasLoader::loadModel('Metadata')->addMetadataValue(
                $lastRevision,
                MIDAS_METADATA_TEXT,
                'mrbextrator',
                'slicerdatastore',
            ...
            if (!$metadataDao) {
                MidasLoader::loadModel('Metadata')->addMetadata(MIDAS_METADATA_TEXT, 'mrbextrator', 'category', '');
            }

            MidasLoader::loadComponent('Metadata', 'slicerdatastore')->setRevisionCategories(
                $lastRevision,
                $categories
            );
        ...
        // Delete temps folder.
        $rootDirectory = opendir($bitstreamsFolder) or die('Error');
        while ($Entry = @readdir($rootDirectory)) {
            if (!is_dir($bitstreamsFolder.'/'.$Entry) && $Entry != '.' && $Entry != '..') {
